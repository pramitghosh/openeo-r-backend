...
R2Generic = R6Class(
  ...
  public = list(
    ...
    initialize = function(granules = NULL, legend_counter = NULL, legend = NULL)
    {
      ...
      self$legend = matrix(ncol = 5, nrow = length(granules) * t_bands) # For filename, timestamp and band - more could be added later by increasing the value of ncol
      colnames(self$legend) = c("filename", "time_index", "timestamp", "band_index", "band")
    ...
    write_time = function(time_observation, t_num, out_path)
    {
      ...
      legend_timeslice = matrix(ncol = 5, nrow = bands_num)
      colnames(legend_timeslice) = c("filename", "time_index", "timestamp", "band_index", "band")
      ...
      for(j in 1:bands_num)
      {
        ...
        self$legend[self$legend_counter + j,] = c(filename, time_index, timestamp, j, band_names[j])
      }
